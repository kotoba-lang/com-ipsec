(ns ipsec.main
  "Kotodama WASM entrypoint for the Ipsec clean-room actor (L4) — Clojure port.
  ...
  facts (`ipsec.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(def entity-specs
  [{:entity "IkeHeader" :plural "ikeheaders" :id-prefix "ipsec_ike"
    :fields [:initiatorSpi :responderSpi :nextPayload :majorVersion :minorVersion :exchangeType :flags :messageId :totalLength]
    :required [:initiatorSpi :responderSpi]
    :coerce {:initiatorSpi :int :responderSpi :int :nextPayload :int :majorVersion :int :minorVersion :int :exchangeType :int :flags :int :messageId :int :totalLength :int}
    :refs {}}
   {:entity "Payload" :plural "payloads" :id-prefix "ipsec_pay"
    :fields [:nextPayload :critical :payloadLength :payloadType]
    :required [:nextPayload :critical]
    :coerce {:nextPayload :int :critical :bool :payloadLength :int :payloadType :int}
    :refs {}}
   {:entity "SecurityAssociation" :plural "securityassociations" :id-prefix "ipsec_sec"
    :fields [:payloadType :proposals]
    :required [:payloadType :proposals]
    :coerce {:payloadType :int :proposals :int}
    :refs {}}
   {:entity "Proposal" :plural "proposals" :id-prefix "ipsec_pro"
    :fields [:number :protocolId :spiSize :transformCount :spi]
    :required [:number :spiSize]
    :coerce {:number :int :protocolId :int :spiSize :int :transformCount :int}
    :refs {}}
   {:entity "Transform" :plural "transforms" :id-prefix "ipsec_tra"
    :fields [:type :transformId :transformLength :attributes]
    :required [:type :transformLength]
    :coerce {:type :int :transformId :int :transformLength :int}
    :refs {:transformId "Transform"}}
   {:entity "Notify" :plural "notifies" :id-prefix "ipsec_not"
    :fields [:protocolId :spiSize :notifyMessageType :spi :notificationData]
    :required [:spiSize :notifyMessageType]
    :coerce {:protocolId :int :spiSize :int :notifyMessageType :int}
    :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"ipsec.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "ipsec-compat" :tier tier :entities entities} 200])
